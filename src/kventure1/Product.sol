...
contract Product is Initializable, OwnableUpgradeable, PackageInfoStruct{
    ...
    function order(
        ...
        for(uint i=0;i<idArr.length;i++){
            require(mProduct[idArr[i]].price >0," product id does not exist ");
            uint amount = mProduct[idArr[i]].price * quaArr[i];
            memPrice += amount;
        }
        ...
        // mTokenId[orderId] =codesArr;
        mOrderArr[to].push(orderId);
        // Order memory orderArr = new Order[]()
        ...
        // mOrder[orderId] = orderPro;
        {
            // Flow Member Price
            address referrerWallet; 
            address menberBonusWallet = to; 
            uint256 transferedAmountToParner;
            uint256 actualPrice = memPrice;
            if (!ref.CheckActiveMember(to)) {
                actualPrice = memPrice * 122 / 100;
                transferedAmountToParner = memPrice * 22 / 100;
               
                if (codeRef != bytes32(0)) {
                    menberBonusWallet = ref.GetRefCodeOwner(codeRef); 
                    require(menberBonusWallet != address(0), "Kventure: Invalid Code Ref");
                    actualPrice = memPrice * 122 * 92 / (100**2);

                    transferedAmountToParner = memPrice * 736 / (100**2);

                    if (ref.CheckActiveMember(menberBonusWallet)) {
                        referrerWallet = menberBonusWallet;
                    } else {
            ...
            ref.TransferCommssion(menberBonusWallet, memPrice);
            // Transfer Commission For Flow Member Price
            if (referrerWallet != address(0)) {
                ref.TransferCommissionByProduct(referrerWallet, memPrice * 488 / (100**2)); // 4% of memPrice
            }
            if (transferedAmountToParner > 0) {
                uint256 partnerDebt = ref.PartnerDebt();
                if (transferedAmountToParner >= partnerDebt){
                    ref.TransferCommissionByProduct(ref.partnerWallet(), transferedAmountToParner - partnerDebt);
                }
                ref.PartnerPaymentDebt(transferedAmountToParner);
            }
        }
    ...
    function adminAddProduct(
        ...
        bytes32 idPro = keccak256(abi.encodePacked(_imgUrl,_price,_desc));
