...
contract KVenture is Initializable, OwnableUpgradeable,PackageInfoStruct {
    ...
    mapping(address => uint256) public totalRetailBonus;
    ...
    function TransferCommssion(address buyer, uint256 price, uint256 diffPrice) external onlyPackController {
        ...
        // _payGoodSaleBonus(buyer,diffPrice);   
        _bonusForDiamond(price);
        _bonusForCrownDiamond(price);
    }

    function TransferRetailBonus(address buyer, uint256 bonus)external onlyPackController {
        console.log("buyer la",buyer);
        IMasterPool(masterPool).transferCommission(buyer, bonus/2);
        totalRevenues[buyer] += bonus/2;
        totalRetailBonus[buyer] += bonus/2;
        emit PayBonus(buyer, bonus/2, block.timestamp,"Retail"); 
    }
    ...
    function _transferSaleCommission(address buyer, uint256 price) internal {
        ...
        {   
            ...
            if (_isValidLevel(parent,index+1)) {   
                ...
                console.log("commAmount la",commAmount);
                if(commAmount>0){
                    if(isActive[parent] == false){
                        IMasterPool(masterPool).transferCommission(DAO_DT, commAmount);
                    }else{
                        IMasterPool(masterPool).transferCommission(parent, commAmount);
                        totalRevenues[parent] += commAmount;
    ...
    function _payGoodSaleBonus(address _menberBonusWallet, uint256 _bonus) internal  {
        uint256 rate =0;
        for (uint256 j = 0; j < levelCareerBonus.length; j++) {
                if (_bonus*50/100 < levelCareerBonus[j]*usdtDecimal) {
                    break;
                } else {
                    rate += additionalCom;
                }
        ...
        console.log("rate la",rate);
            if(rate >0){
                ...
                uint256 transferredAmount = (_bonus* rate /10**3) ;
                bool success;
                if(isActive[_menberBonusWallet] == false){
                    ...
                    success = IMasterPool(masterPool).transferCommission(_menberBonusWallet, transferredAmount);
                    emit PayBonus(_menberBonusWallet, transferredAmount, block.timestamp,"GoodSale");            
                    require(success, "Failed transfer good sale bonus");
                }
                totalCareerBonus[_menberBonusWallet] += transferredAmount;
