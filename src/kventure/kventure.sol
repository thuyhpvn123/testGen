...
contract KVenture is Initializable, OwnableUpgradeable,PackageInfoStruct {
    ...
    uint256 public RETAIL_BONUS_RATE;

    constructor() payable {}
    function initialize(address _usdt, address _masterPool,address _binaryTree,address _root, address _wallet, address _partnerWallet, address _kCode, address _mtn) public initializer {
        usdt = _usdt;
    ...
    function _transferDirectCommission(address buyer,uint256 _firstFee) internal {
        ...
        if(isActive[parentDirect] == false){
            IMasterPool(masterPool).transferCommission(DAO_DT, commAmountFA);
        } else {
        ...
        {   
            if (parentMatrix == address(0)) {
                IMasterPool(masterPool).transferCommission(DAO_DT, maxAmountBonus - totalAmountTransfer);
                break;
            }
            ...
            if (_isValidLevel(parentMatrix,index) && isActive[parentMatrix] == true) {   
                ...
                IMasterPool(masterPool).transferCommission(DAO_DT, commAmount);
            }
    ...
    function _transferMatchingBonus(address buyer,uint256 amount) internal returns (uint) {
        ...
        {   
            ...
            if(gen.Silver<=2){matchingRate += comMatchingRate[1];}
            ...
            if(matchingRate>0){
                if(isActive[parent] == false){
                    success = IMasterPool(masterPool).transferCommission(DAO_DT, commAmount);
    ...
    function _transferMatrixBonus(address buyer,uint256 amount) internal {
        ...
        // Company
        success = IMasterPool(masterPool).transferCommission(DAO_DT, matrixCompanyRateBonus * amount / 10**3);
        require(success, "Failed transfer matrix commission"); 

        for (uint index = 0; index < maxLineForMatrixBonus; index++) 
        {   
            if (parent == address(0)) {
                success = IMasterPool(masterPool).transferCommission(DAO_DT, maxMatrixBonus - totalAmountTransfer);
                require(success, "Failed transfer matrix commission"); 
                break;
            }
    ...
    function TransferCommssion(address buyer, uint256 price, uint256 diffPrice) external onlyPackController {
        totalSale[buyer] += price;
        // 50% to mtn
        IMasterPool(masterPool).transferCommission(MTN, price * MTN_RATE / 10**3);
        // 18% to mtn
        PartnerPaymentDebt(price * DT_RATE / 10**3);
        // 29.5% to bonus, because bonus max is 130% diffPrice then if diffPrice >= 22.692..% price we will suffer a loss
        // The price difference is decided by the BD (func adminAddProduct) 
        _transferSaleCommission(buyer, diffPrice);
        totalSaleRevenue += price;
        // 2% to diamond
        _bonusForDiamond(price);
        // 0.5% to crown diamond
        _bonusForCrownDiamond(price);
    }

    function TransferRetailBonus(address _address, uint256 diffPrice) external onlyPackController{
        uint256 bonus = diffPrice * RETAIL_BONUS_RATE / 10**3;
        IMasterPool(masterPool).transferCommission(_address, bonus);
        emit PayBonus(_address, bonus, block.timestamp,"Sale");   
        
        uint256 rate = 0;
        for (uint256 j = 0; j < levelCareerBonus.length; j++) {
            if (bonus < levelCareerBonus[j]*usdtDecimal) {
                break;
            } else {
                rate += additionalCom;
            }
        }
        totalRevenues[_address] += diffPrice * rate / 10**3;
    }
    ...
    function _transferSaleCommission(address buyer, uint256 price) internal {
        ...
        //pay 50% diff price for F1
        commAmountFA = (commissionRate[0]*price) / 10**3;
        if(isActive[parentDirect] == true){
            IMasterPool(masterPool).transferCommission(parentDirect, commAmountFA);
            emit PayBonus(parentDirect, commAmountFA, block.timestamp,"Sale");   
        } 
        totalSaleBonus[parentDirect] += commAmountFA;
        uint256 rate;
        for (uint256 j = 0; j < levelCareerBonus.length; j++) {
            if (commAmountFA < levelCareerBonus[j]*usdtDecimal) {
                break;
            } else {
                rate += additionalCom;
            }
        }
        totalRevenues[parentDirect] += price * rate / 10**3;

        // 30% bonus in binary tree (matrix)
        for (uint index = 1; index < commissionRate.length; index++) 
        {   
            if (parentMatrix == address(0)) {
                break;
            }

            if (_isValidLevel(parentMatrix,index+1)) {   
                // Pay commission by subscription
                commAmount = commissionRate[index]*price/10**3;
                if(commAmount > 0){
                    if(isActive[parentMatrix] == true){
                        ...
                        totalRevenues[parentMatrix] += rate * price / 10**3;
                        emit PayBonus(parentMatrix, commAmount, block.timestamp,"Sale");       
                    }
    ...
    function payGoodSaleBonusWeekly() external onlyAdmin returns (bool) {
        ...
        while (front < back) {
            ...
            if(totalRevenues[current] >0){
                // Pay commission good sale
                if(isActive[current] == true){
                    IMasterPool(masterPool).transferCommission(current, totalRevenues[current]);
                    emit PayBonus(current, totalRevenues[current], block.timestamp,"GoodSale");            
                }
                totalGoodSaleBonus[current] += totalRevenues[current];
                totalCareerPayment += totalRevenues[current];
        ...
        if(len > 0){
            for(uint i=0;i<len;i++){
                ...
                if(isActive[diamondArr[i]] == false){
                    success = IMasterPool(masterPool).transferCommission(DAO_DT, transferredAmount);
            ...
            success = IMasterPool(masterPool).transferCommission(DAO_DT, revenue * diamondRate / 10**3);
    ...
    function _bonusForCrownDiamond(uint revenue) internal{
        ...
        if(len > 0){
            ...
            success = IMasterPool(masterPool).transferCommission(DAO_DT, revenue * crownDiamondRate / 10**3);
 ...
 function updateRankDaily() external onlyAdmin returns(bool){
    ...
    function PartnerPaymentDebt(uint256 amount) internal returns(bool) {
        if (amount >= DT_DEBT) {
            DT_DEBT = 0;
            IMasterPool(masterPool).transferCommission(DT, amount - DT_DEBT);
        } else {
            DT_DEBT -= amount;
        }
        return true;
